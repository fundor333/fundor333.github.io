...
def fetch_event(event_id):
    ...
    data = r.json()
    data["id"] = event_id
    data["now"] = (datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=36500)).strftime("%Y-%m-%dT%H:%M:%S")
    return data


def convert_dict_to_json(data):
    # convert the json to a pretty json and save it in data/event/event_id.json
    Path("data/event").mkdir(parents=True, exist_ok=True)
...
def convert_dict_to_post(data):
    ...
    event = data["data"]["event"]
    title = event["title"]
    date_time_file = datetime.datetime.fromisoformat(event["dateTime"].replace("Z", "")).strftime("%Y-%m-%d")
...
---\n"""
    ...
    Path("content/event").mkdir(parents=True, exist_ok=True)
...
def add_to_memory(text: str):
    # Save in action_script//data/memory_meetup.txt, if not already present, the url pass as argument
    Path("action_script/data").mkdir(parents=True, exist_ok=True)
    memory_file = "action_script/data/memory_meetup.txt"
    if not Path(memory_file).exists():
        with Path.open(memory_file, "w") as f:
            f.write("")

    with Path.open(memory_file) as f:
        memory = f.read().splitlines()
    if text not in memory:
        with Path.open(memory_file, "a") as f:
            f.write(text + "\n")
...
def fetch_event_from_json(event_id):
    ...
    if not Path(filename).exists():
        print(f"No JSON file found for event ID {event_id}. Now Fetching from Meetup...")
...
def main(
    ...
    if memory:
        if Path("action_script/data/memory_meetup.txt").exists():
